...
const onChangeAddress = (item: AddressItem) => {
  ...
  addressStore.changeAddress(item)
  uni.navigateBack()
}
</script>
...
<template>
  <view class="viewport">
          ...
          <uni-swipe-action-item class="item" v-for="item in addressList" :key="item.id">
            <view class="item-content" @tap="onChangeAddress(item)">
              ...
              <navigator
                class="edit"
                hover-class="none"
                :url="`/pagesMember/address-form/address-from?id=${item.id}`"
                @tap.stop="() => {}"
              >
                修改
              </navigator>
            ...
            <template v-slot:right>
              <button @tap="onDeleteChange(item.id)" class="delete-button">删除</button>
            </template>
    ...
    <!-- 添加按钮 -->
    <view class="add-btn">
      <navigator hover-class="none" url="/pagesMember/address-form/address-from">
        新建地址
      </navigator>
    </view>
  </view>
