...
onReady(() => {
  ...
  pageInstance.animate(
    '.navbar .title',
    [{ color: 'transparent' }, { backgroundColor: '#f8f8f8' }],
    1000,
    {
      scrollSource: '#scoller',
      timeRange: 1000,
...
const onPayMent = async () => {
  ...
  if (import.meta.env.DEV) {
    ...
    // 获取支付参数
    const res = await getPayWxPayMiniPay(query.id)
    // 生产环境，使用微信支付
    await wx.requestPayment(res.result)
  }
  // 关闭当前页，再跳转支付结果页
  uni.redirectTo({ url: `/pagesOrder/peyment/peyment?id=${query.id}` })
}
...
const onOrderConfirm = () => {
  uni.showModal({
    content: '保障您的权益，请收到货确认无误后，再确认收货',
    ...
    <template v-if="order?.orderState === OrderState.DaiFuKuan">
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待评价状态: 展示去评价 -->
          <view class="button" v-if="order.orderState === OrderState.DaiShouHuo"> 去评价 </view>
